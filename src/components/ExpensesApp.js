...
class ExpensesApp extends Component {
  ...
  validate = () => {
    ...
    if(form){
      ...
      // Esta condicion solo aplica a la venta (Expenses)
      else {
        for(let key in option){
          if(form["productId"] === option[key].value){
            if(form["amount"] > option[key].amountTotal){
              validityStatus = false
              errors["amount"] = "La cantidad es mayor a lo que hay en el inventario."
            }
          }
        }
      }
    ...
    axios.get(url + "/inventory/all").then(response=>{
      ...
      for (let key in response.data){
        options.push({
            ...
            amountTotal: response.data[key].amountTotal,
  ...
  peticionGetExpenses=()=>{
    axios.get(url + "/expenses/all").then(response=>{
      ...
      this.setState({productListExpenses: response.data});
    }).catch(error=>{
      console.log(error.message);
    })
  }
  ...
  peticionPost=async()=>{
    if(this.validate()){
    await axios.post(url + "/expenses/add",this.state.form).then(response=>{
        this.modalInsertar();
  ...
  return (
    ...
    <div className='py-4 mb-2 bg light d-flex justify-content-center'>
        <button className="btn btn-primary btn-lg btn-block" onClick={()=>{this.setState({form: null, tipoModal: 'insertar'}); this.modalInsertar()}}>AGREGAR VENTA</button>
      </div>
      <div className='py-4 mb-2 bg light'>
        <h1 className='h2 mb-0'>Lista de productos vendidos ({ this.state.productListExpenses.length })...</h1>
      </div>
      <Table striped bordered hover size="sm">
        ...
        <tbody>
          {this.state.productListExpenses.map(product=>{
            return(
              <tr key={product.id}>
              <td>{product.date}</td>
